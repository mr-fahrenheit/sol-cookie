...
export async function stake(program: Program<Cookie>, authority: PublicKey, player: Keypair, mint: PublicKey, amount: number) {
    ...
    expect(gameDiff).to.be.equal(amount);
    expect(rewardsDiff).to.be.equal(0);
    expect(stakedDiff).to.be.equal(amount);
}

export async function stakeAll(program: Program<Cookie>, authority: PublicKey, player: Keypair, mint: PublicKey) {
    const connection = program.provider.connection;
    const gamePDA = getGamePDA(program, authority);
    ...
    expect(stakedDiff).to.be.equal(playerBalanceBefore);
}